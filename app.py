...
#set matplotlib to noninteractive backend
matplotlib.use('Agg')
...
@app.route("/predict")
def predict():
    stock=request.args.get("stock","DOGE-USD")
    ...
    plt.legend()
    original_plot = matplot_to_html(fig1)
    ...
    plt.xlabel("Date")
    plt.ylabel("Close Price")
    predicted_plot = matplot_to_html(fig2)
    ...
    plt.grid(alpha=0.3)
    plt.legend()
    future_plot = matplot_to_html(fig3)
